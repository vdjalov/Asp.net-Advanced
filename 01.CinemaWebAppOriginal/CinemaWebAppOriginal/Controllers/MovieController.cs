...
namespace CinemaWebAppOriginal.Controllers
{
    ...
    public class MovieController : BaseController
    {
        ...
        public async Task<IActionResult> SoftDelete(int id)
        {
            ...
            bool checkIfMovieExists = await this.movieService.CheckIfMovieExists(id);

            if (checkIfMovieExists == false)
            {
                return RedirectToAction(nameof(Manage));
            }

            DeleteMovieViewModel viewModel = await this.movieService.GetDeleteMovieViewModelByIdAsync(id);
            if (viewModel == null)
            {
                ModelState.AddModelError(string.Empty, "Failed to delete the movie. Please try again.");
                return RedirectToAction(nameof(Manage));
            }
        ...
        public async Task<IActionResult> SoftDeleteConfirmed(int id, DeleteMovieViewModel viewModel)
        {
            string userId = this.GetUserId();
            bool isUserManager = await this.managerService.IsUserAManager(userId);
            if (!isUserManager)
            ...
            if (id != viewModel.Id) // no idea if that is necessary generated by the AI
            {
                TempData["ErrorMessage"] = "Movie ID mismatch. Please try again.";
                return RedirectToAction(nameof(SoftDelete), new { id = id});
            }

            bool deleteResult = await this.movieService.SoftDeleteMovieAsync(id);

            if (!deleteResult)
            {
                TempData["ErrorMessage"] = "Failed to delete the movie. Please try again. Movie might still be showing in cinemas.";
                return RedirectToAction(nameof(SoftDelete), new { id = id});
            }
            return RedirectToAction(nameof(Manage));
